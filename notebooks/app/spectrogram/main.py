...
PLOTARGS = dict(tools="", outline_line_color='#595959', sizing_mode="stretch_width")
...
waterfall_plot = figure(plot_width=1000, plot_height=300,
                        x_range=[0, NUM_GRAMS],
                        y_range=[0, MAX_FREQ_KHZ], **PLOTARGS)

# option to swap axis for waterfall, so x-axis is FREQUENCY for all charts
# waterfall_plot = figure(plot_width=1000, plot_height=300,
#                         x_range=[0, MAX_FREQ_KHZ], y_range=[0, NUM_GRAMS], **PLOTARGS)
...
waterfall_plot.xaxis.axis_label="Time"
waterfall_plot.yaxis.axis_label="Frequency (KHz)"
...
signal_plot.xaxis.axis_label="Time (last 100ms window)"
signal_plot.yaxis.axis_label="Strength (dB)"

spectrum_source = ColumnDataSource(data=dict(f=[], y=[]))
spectrum_plot = figure(plot_width=600, plot_height=200, title="Spectrum",
                       background_fill_alpha=0.9,
                       y_range=[10**(-4), 10**3], x_range=[0, MAX_FREQ_KHZ],
                       y_axis_type="log", **PLOTARGS)
spectrum_plot.background_fill_color = "#eaeaea"
spectrum_plot.line(x="f", y="y", line_dash="dotted", line_color="firebrick", source=spectrum_source)
spectrum_plot.xaxis.axis_label="Frequency (KHz)"
spectrum_plot.yaxis.axis_label="Strength (dB)"
...
eq = figure(plot_width=400, plot_height=400, title="Equalizer View - Realtime Frequency",
            background_fill_color="#999999", background_fill_alpha=0.9,
            x_axis_type=None, y_axis_type=None,
            x_range=[-20, 20], y_range=[-20, 20], **PLOTARGS)
# eq.background_fill_color = "#eaeaea"

# old fill_color:  "#024768"
eq.annular_wedge(x=0, y=0, fill_color="#cc6a66", fill_alpha="alpha", line_color=None,
                 inner_radius="inner", outer_radius="outer", start_angle="start", end_angle="end",
                 source=eq_source)

freq = Slider(start=1, end=MAX_FREQ, value=MAX_FREQ, step=1, title="Frequency (Hz)")
